game = {
  ...
  init_gui: function(){
    ...
    let menubutton = gui.createElement('button',{
      id: "menubutton",
      className: 'numberbutton',
      innerText: '☰',
      onclick: function(){
    ...
    buttonfield.appendChild(menubutton)
    let sm = document.getElementById('startmenu')
    sm.appendChild(gui.createElement('button',{
      innerText: 'new Sudoku',
      onclick: function(){
        game.active_sudoku_nr++
        if(game.active_sudoku_nr>sudokus.length){
          alert('no more sudokus left to solve')
          return
        }
        game.init_sudoku(sudokus[game.active_sudoku_nr])
        sm.close()
      },
    }))
  ...
  init_sudoku: function(sudokuarr){
    this.sudoku = JSON.parse(JSON.stringify(sudokuarr || sudokus[0]))
  ...
  save_state: function(){
    let saveobj = {
      original_sudoku: sudokus[this.active_sudoku_nr],
      filled_sudoku: this.sudoku,
    }
    localStorage.setItem('sudoku', JSON.stringify(saveobj))
  },
  load_state: function(){
    let saveobj = localStorage.getItem('sudoku')
    if(saveobj)saveobj = JSON.parse(saveobj)
    this.init_sudoku(saveobj.original_sudoku)
    for(let i=0;i<81;i++){
      if(saveobj.filled_sudoku[i]){
        this.sudoku[i]=saveobj.filled_sudoku[i]
        document.getElementById('field'+i).firstChild.innerText = saveobj.filled_sudoku[i]
      }
    }
  },
}
